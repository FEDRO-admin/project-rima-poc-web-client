...
**Task:** 655  
**Branch:** feature/584-historical_state  
**Date:** July 2026
...
The Create system allows users to add new features to an editable `FeatureLayer` directly on the map. It is triggered from the **Table of Contents (TOC)** via a "Create" action on any `FeatureLayer` entry, and runs as a standalone overlay panel (`<rima-create-form>`) that is always present in the map layout but only visible when `createStore.active()` is `true`.

The flow follows four ordered phases:

1. **Select a drawing tool** — user picks the geometry type for the new feature.
2. **Draw** — user places the geometry on the map via `SketchViewModel`.
3. **Adjust** — user fine-tunes placement (move/rotate) immediately after drawing.
4. **Fill attributes & save** — user completes the attribute form, optionally manages reference points, then saves.
...
The TOC (`TocComponent`) adds a **"Create"** button action to every `FeatureLayer` in the `<arcgis-layer-list>`. When clicked, `TocComponent.createFeature(layer)` runs:

1. Loads the layer (`layer.load()`).
2. Resolves the subtype field via `getSubtypeFieldName(layer)`.
3. Determines the default subtype value by querying an existing feature, then falling back to `getSubtypeCodeFromLayerName()` or `getDefaultSubtypeCode()`.
4. Calls `CreateStore.activate(layer, subtypeField?, subtypeValue?)`.
...
- **Cancels any active edit session** if `editEffects.editing()` is `true`.
...
## Geometry Lifecycle (`CreateGeometryService`)

### Phase 1 — Draw

`startDrawing(layer, tool?)` sets up a `SketchViewModel` in **create mode**:
...
- Automatically enters **adjust mode** (`startAdjusting()`) — switches to `update` tool with `enableRotation: true` and `toggleToolOnClick: true` so the user can move and rotate the geometry.

**Drawing tools by geometry type:**
...
While `adjusting` is `true`, the form shows `"Adjust geometry..."` with Undo / Redo / **Confirm** (✓) / **Redraw** (↺) buttons.

- **`undo()` / `redo()`** — delegate to `SketchViewModel`.
...
After `confirmPlacement()`, the form shows `"Geometry added"` with **Edit** and **Redraw** buttons.
...
2. Resolves the effective domain per field — subtype-specific domain takes precedence over field-level domain.
3. Produces `AttributeEditField[]` with `fieldType`: `string | integer | double | date | coded-value | guid`.
4. Default values are pre-populated via `buildDefaultAttributes()` from each field's `defaultValue`.
...
**`canSave`:** `geometry != null && !saving`.

Clicking **"Create Feature"** sets `confirmAction = 'save'`, showing:  
**"Are you sure you want to create this feature?"**

On confirmation, `CreateService.saveAndOpenInPopup()` runs:

1. Calls `save()`:
   - `createGeometryService.cleanup()` — destroys the `SketchViewModel` and removes the sketch layer.
   - `buildCreatePayload()` — collects all mutable, non-objectId attributes plus the subtype field value.
   - `layer.applyEdits({ addFeatures: [newGraphic] })`.
   - Returns the new feature's `objectId`.
2. Calls `layer.refresh()` to update the map display.
3. Re-queries the new feature: `layer.queryFeatures({ objectIds: [objectId], outFields: ['*'], returnGeometry: true })`.
4. Calls `ReferencePointService.saveAll(parentId)` for any pending reference points.
5. Resets the store (`createStore.reset()`).
6. Opens the popup with the newly created feature graphic.
...
Clicking **"Cancel"** (or pressing Escape) calls `requestCancel()` / `requestClose()`:
